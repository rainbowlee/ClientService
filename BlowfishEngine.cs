    ...
    private static uint[] KP =
{
    ...
    private static readonly int BLOCK_SIZE = 8; // bytes = 64 bits
    ...
    public int processBlock(byte[] inb, int inOff, byte[] outb, int outOff)
    {
      if (workingKey == null)
      {
        throw new Exception("Blowfish not initialised");
      }
      if ((inOff + BLOCK_SIZE) > inb.Length)
      {
        throw new Exception("input buffer too short");
      }
      if ((outOff + BLOCK_SIZE) > outb.Length)
      {
        throw new Exception("output buffer too short");
      }
      if (encrypting)
      {
        encryptBlock(inb, inOff, outb, outOff);
      }
      else
      {
        decryptBlock(inb, inOff, outb, outOff);
      }
      return BLOCK_SIZE;
    }
    ...
    private void setKey(byte[] key)
    {
      ...
      for (int i = 0; i < P_SZ; i++)
      {
        ...
        for (int j = 0; j < 4; j++)
        {
          ...
          data = (uint)((data << 8) | (key[keyIndex++] & 0xff));
    ...
    private uint bytesTo32bits(byte[] b, int i)
    {
      return (uint)(((b[i + 3] & 0xff) << 24) | ((b[i + 2] & 0xff) << 16) | ((b[i + 1] & 0xff) << 8) | ((b[i] & 0xff)));
    }

    private void bits32ToBytes(uint inb, byte[] b, int offset)
    {
      b[offset] = (byte)inb;
      b[offset + 1] = (byte)(inb >> 8);
      b[offset + 2] = (byte)(inb >> 16);
      b[offset + 3] = (byte)(inb >> 24);
    }
